...
public class StudentController {
    ...
    // Theem mowis
    @PostMapping
    public ResponseEntity<DataResponse<Student>> addStudent(@Valid @RequestBody StudentAddDto request){
        return new ResponseEntity<>(studentService.addStudent(request),HttpStatus.CREATED);
    }

    @PutMapping("/{id}")
    public ResponseEntity<DataResponse<Student>> updateStudent(@PathVariable Integer id, @RequestBody StudentUpdateDto request) throws NotFoundException {
